...
Hourly Day-Ahead LMP forecasting for PJM SOUTH aggregate node using ~30 active data sources, ~51 engineered features, LightGBM + Ridge ensemble, and spike classification.
...
- **Gas markets via GasClient** (5 datasets): Columbia Gas (TCO) daily spot, PJM WHub prompt-month forward curve, Transco Zone 5 gas forward curve, Dominion South Point spot, TETCO M3 spot
- **PJM API** (4 new feeds): ancillary service prices (RegA, RegD, Sync Reserve), 5-minute marginal CO₂ emission rates (aggregated hourly), binding transmission constraints with shadow prices, and PJM instantaneous load (aggregated hourly)
- **EIA** (2 free feeds): Henry Hub natural gas spot price, weekly underground gas storage
...
| `n_binding_constraints_h` | Hourly binding transmission constraint count |
| `max_shadow_price_d1` | D-1 maximum shadow price ($/MWh) — congestion magnitude |
| `congestion_regime` | Binary: high-congestion regime flag |
| `storage_vs_5yr_avg` | Weekly gas storage vs 5-year seasonal average (Bcf) |
| `storage_injection_rate` | Week-over-week storage change (Bcf) — supply signal |
| `dominion_z5_spread` | Dominion South − Transco Z5 price spread |
| `tetco_z5_spread` | TETCO M3 − Transco Z5 price spread |
...
With the full expanded data stack: **~15–20% MAE reduction** versus the base model (primarily driven by GHI solar prediction, forward curves, PJM ancillary service signals, and binding transmission constraints).